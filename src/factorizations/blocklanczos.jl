...
struct BlockLanczosIterator{F,T,O<:Orthogonalizer,S<:Real} <: KrylovIterator{F,T}
    ...
    orth::O
    qr_tol::S
    function BlockLanczosIterator{F,T,O,S}(operator::F, x₀::Block{T}, maxdim::Int, orth::O,
                                           qr_tol::S) where {F,T,O<:Orthogonalizer,S<:Real}
        return new{F,T,O,S}(operator, x₀, maxdim, orth, qr_tol)
    end
end
function BlockLanczosIterator(operator::F, x₀::Block{T}, maxdim::Int,
...
function initialize(iter::BlockLanczosIterator;
    ...
    is_draft &&
        @warn "Block size may be more than operator's rank. Please decrease Krylovdim or increase qr_tol."
